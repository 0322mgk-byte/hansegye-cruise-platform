...
**레퍼런스 상품:** `data/products/alaska-emerald-princess/` (각 파일 생성 전에 반드시 해당 레퍼런스 파일을 실제로 읽는다)
**타입 정의:** `data/types.ts`
...
| 10 | `pricing-data.ts` | 🔒 | 기본 그대로 복사. 사용자가 가격 제공 시에만 해당 필드 교체 | `references/data-structure.md` Section 8 |
...
사용자가 제공한 일정 원본은 **파싱의 입력 소스**로만 취급한다 (parsed-data.md에 원본 보존). 실제 `schedule-days-data.ts` 생성 시에는 알래스카 레퍼런스(`data/products/alaska-emerald-princess/schedule-days-data.ts`)의 **형식과 구조**에 맞춰 정규화한다.
...
| 필드 | 규칙 |
|------|------|
| `slug` | `{지역명}-{선박명}` 영문 소문자, 하이픈 구분 (예: `alaska-emerald-princess`) |
...
※ 상품 페이지 내 경로 표시(`routeCities`, data-structure.md Section 2)와는 별개 규칙이다.
...
데이터 파일 생성 전에 **알래스카 레퍼런스 파일(`data/products/alaska-emerald-princess/`)을 실제로 읽고**, 다음 참조 문서에서 교체할 필드 목록을 확인:
