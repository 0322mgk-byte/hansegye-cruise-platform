...
OG 이미지는 직접 ffmpeg로 크롭 변환하고, 나머지는 `scripts/batch-convert.js`로 처리한다.
...
### 3-A: OG 이미지 (1200×630 크롭 + webp 변환)

OG 이미지(`og.*`)는 `batch-convert.js`를 사용하지 않고, 직접 ffmpeg 명령으로 **1200×630px 중앙 크롭 + webp 변환**을 수행한다.

**원본이 webp가 아닌 경우** (png, jpg 등):
```bash
ffmpeg -i "public/products/{상품명}/og.{원본확장자}" -vf "scale=1200:630:force_original_aspect_ratio=increase,crop=1200:630" -c:v libwebp -quality 80 "public/products/{상품명}/og.webp" -y
```

**원본이 이미 webp인 경우** (크롭만 수행):
```bash
ffmpeg -i "public/products/{상품명}/og.webp" -vf "scale=1200:630:force_original_aspect_ratio=increase,crop=1200:630" -c:v libwebp -quality 80 "public/products/{상품명}/og_crop.webp" -y
```
→ 원본 `og.webp` 삭제 → `og_crop.webp`를 `og.webp`로 이름 변경

- `scale=1200:630:force_original_aspect_ratio=increase` → 짧은 쪽이 목표 치수에 맞도록 확대 (비율 유지)
- `crop=1200:630` → 중앙 기준 크롭하여 정확히 1200×630 출력
- quality 80, 크기 제한 없음

**변환 결과 기록** — 5단계, 7단계에서 사용:
- 원본과 다른 확장자 (예: `og.png` → `og.webp`): `CONVERTED`로 기록
- 원본이 이미 webp: `RECOMPRESSED`로 기록
...
**3-A 변환 결과와 3-B 스크립트 출력을 모두 저장해둔다 — 5단계, 7단계에서 사용한다.**
