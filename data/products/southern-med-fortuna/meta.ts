import type { ProductMeta } from '@/data/types';

export const meta: ProductMeta = {
  slug: 'southern-med-fortuna',
  name: '코스타 포르투나호 남부 지중해 크루즈 8박 10일',
  shortDescription: '이스탄불 · 미코노스 · 크레타 · 로도스 · 산토리니 · 아테네',
  thumbnailImage: 'https://pub-f396f9b021ba403fb605a64e3824e612.r2.dev/products/southern-mediterranean-cruise/og.webp',
  ogImage: 'https://pub-f396f9b021ba403fb605a64e3824e612.r2.dev/products/southern-mediterranean-cruise/og.webp',
  ogTitle: '크루즈가 자유여행보다 싸다고?',
  ogDescription: '이스탄불부터 산토리니까지, 숙박·식사·이동 다 포함된 에게해 크루즈 8박',
  cardDepartureLabel: '',
  cardTitle: '남부 지중해 크루즈 8박 10일',
  cardShipName: '코스타 포르투나호 (COSTA Fortuna)',
  cardRoute: '이스탄불 > 미코노스 > 헤라클리온 > 산토리니 > 아테네',
  cardPrice: '',
};
